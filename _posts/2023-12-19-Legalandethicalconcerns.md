...
comments: false
layout: post
title: Legal and Ethical Concerns of Technology 
description: A lesson on the legal and ethical concerns of technology. homework
type: hacks
...
CC0 puts the creator's work in the public domain, and allows anyone to copy/modify/distribute the work without asking for permission. This is primarily used for things such as images, music, and text.

Open Source MIT License mostly deals with software/coding projects. Again, users can do pretty much anything with the code (including modifying it) as long as they include the original copyright or license files within their project. So it is actually not much more restrictive than CC0.

Open Source GPL License ensures that all modified versions of the code must also be distributed as open-source and under the same GPL license. This allows the code to remain as always open-source no matter what modifications are made to it.
...
Qualcomm for example has a central business model revolving around development of computer processors that can be used in Android devices. Android devices are the most common in the world, and their OS is based off open-source software, so this is an example of how Qualcomm has used Open Source licenses (those of Android) to develop something unique to build off of that.
...
Under open source licenses, all the software that the businesses create will be open-source, which means that users can modify/use their code at free will without much difficulty or cost. Thus, it is hard for them to generate income this way.

Sometimes they will use something such as an Open Core Model, which means that the core product they offer (like the central part of the software) is free under an open-source license, and offer additional "add-on" features with proprietary licenses that come with costs for the users. In this way, it is easier for them to generate revenue.
...
For personal and team Github repositories, choosing the right license to fit the group's needs is very important especially when considering future plans for the project. Since different projects will have different end results and different goals, it is important to consider whether having the project be open-source or under a Creative Commons license is beneficial. For the CPT project, many of the project's themes/goals are based in the real world (easy example is weather API) and in the future the creators may want to somehow distribute their work. This is when it is vital to choose the most appropriate license for their work as it determines the fate of the project, how it can be used legally and ethically. With a bad choice of license, the potential of the project to gain revenue/recognition for the creators can be voided when others decide to use the work in ways that benefit themselves and remove credit from the original creators.